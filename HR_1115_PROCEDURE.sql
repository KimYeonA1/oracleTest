...
DROP TABLE EMP01;
CREATE TABLE EMP01--복사하기
AS
SELECT * FROM EMPLOYEES WHERE 1 = 1;  --WHERE 조건을 안 주면 모든 행이 다 TRUE가 됨
SELECT * FROM EMP01;
DESC EMPLOYEES;
--부서별로 월급을 인상하는 프로그램이다. 10번 부서는 전원 10%인상, 20번 부서는 전원 20% 인상, 나머지는 동결
...
-- 프로시저를 통해서 사원번호를 입력하면, 사원이름, 사원연봉, 사원월급, 사원직책 매개변수를 통해서 전달(INMODE)
...
END;
/
SELECT FIRST_NAME, JOB_ID, SALARY FROM EMPLOYEES WHERE EMPLOYEE_ID = 100;

DECLARE
    VNAME_ROWTYPE EMPLOYEES%ROWTYPE;
